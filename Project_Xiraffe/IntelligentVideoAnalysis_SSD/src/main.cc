...
/**** Code modified from Vitis-AI VART based "video Analysis" example *****/
/* Original code at: https://github.com/Xilinx/Vitis-AI/tree/master/VART/samples/video_analysis */
...
void RunSSD(vart::Runner* runner, bool& is_running) {
  ...
  while (is_running) {
    ...
    for (size_t i = 0; i < results.size(); ++i) {
      ...
      if (label_index == 1 && confidence >= CAR_THRESHOLD) 
      {  // car
        rectangle(img, cvPoint(x_min, y_min), cvPoint(x_max, y_max),
                  Scalar(0, 0, 255), 1, 1, 0);
      } else if (label_index == 2 && confidence >= BICYCLE_THRESHOLD) 
      {  // bicycle
        rectangle(img, cvPoint(x_min, y_min), cvPoint(x_max, y_max),
                  Scalar(0, 255, 0), 1, 1, 0);
      } else if (label_index == 3 &&
                 confidence >= PERSON_THRESHOLD) 
      {  // person
        rectangle(img, cvPoint(x_min, y_min), cvPoint(x_max, y_max),
...
void Display(bool& is_displaying) 
{
  ...
  while (is_displaying) {
	...
	/*** We display out using gstreamer instead ***/
	cout << "Displaying Image" << endl;
	Mat img=display_queue.top().second;
	// Scale Image to output size if different
